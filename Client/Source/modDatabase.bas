...
Public Sub SaveLocalMap(ByVal MapNum As Long)
    ...
    If FileExistsNew(FileName) Then
        Call Kill(FileName)
    End If

    FileID = FreeFile
    Open FileName For Binary As #FileID
        Put #FileID, , Map(MapNum)
    Close #FileID
End Sub
...
Sub LoadMap(ByVal MapNum As Long)
    ...
    If FileExistsNew(FileName) Then
        FileID = FreeFile
        Open FileName For Binary As #FileID
            Get #FileID, , Map(MapNum)
        Close #FileID
    End If
End Sub

Public Sub SwearFilter_Load(ByVal FileName As String)
    Dim FileID As Integer
    Dim Data As String
    Dim Parse() As String
    Dim Count As Long
    ...
    MAX_SWEARWORDS = CLng(ReadINI("SWEAR_CONFIG", "MaxWords", FileName))

    ' Check to make sure there are swear words in memory.
    If MAX_SWEARWORDS = 0 Then Exit Sub
    
    ' Redim the type to the maximum amount of words.
    ReDim SwearFilter(1 To MAX_SWEARWORDS) As SwearFilterRec

    ' Loop through all of the words.
    For Count = 1 To MAX_SWEARWORDS
        ' Get the bad word from the INI file.
        Data = ReadINI("SWEAR_FILTER", "Word_" & CStr(Count), FileName)
        ...
        If LenB(Data) <> 0 Then
            ...
            SwearFilter(Count).BadWord = Parse(0)
            SwearFilter(Count).NewWord = Parse(1)
        End If
    Next Count
End Sub
...
Public Function SwearFilter_Replace(ByVal Message As String) As String
    ...
    If MAX_SWEARWORDS = 0 Then
        SwearFilter_Replace = Message
        Exit Function
    End If

    ' Loop through all of the words.
    For i = 1 To MAX_SWEARWORDS
        ' Check if the word exists in the sentence.
        If InStr(Message, SwearFilter(i).BadWord) Then
            ' Replace the bad words with the replacement words.
            Message = Replace$(Message, SwearFilter(i).BadWord, SwearFilter(i).NewWord, 1, -1, vbTextCompare)
        End If
    Next i
...
Public Sub SetPlayerMaxHP(ByVal Index As Long, ByVal MaxHP As Long)
    Player(Index).MaxHP = MaxHP
End Sub

Public Sub SetPlayerMaxMP(ByVal Index As Long, ByVal MaxMP As Long)
    Player(Index).MaxMP = MaxMP
End Sub

Public Sub SetPlayerMaxSP(ByVal Index As Long, ByVal MaxSP As Long)
    Player(Index).MaxSP = MaxSP
End Sub
