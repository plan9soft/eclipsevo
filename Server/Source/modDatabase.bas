...
GetVar_Error:
    Call TextAdd(frmServer.txtText(7), "[DB Error] GetVar failed to return the result for '" & Header & ":" & Var & "'.", True)
End Function
...
Public Function FolderExists(inPath As String) As Boolean
    If Not LenB(Dir(inPath, vbDirectory)) = 0 Then
        FolderExists = True
    End If
End Function
...
Public Sub LoadExperience()
    ...
    ' Check and make sure the experience database exists.
    Call CheckExperience

    ' Cache the file name.
    FileName = App.Path & "\Experience.ini"

    ' Load the experience, for each level, into memory.
    For I = 1 To MAX_LEVEL
        Call SetStatus("Loading Experience... " & CStr(I) & "/" & CStr(MAX_LEVEL))
        Experience(I) = Val(GetVar(FileName, "EXPERIENCE", "Exp" & CStr(I)))
    Next I
End Sub

Public Sub CheckExperience()
    Dim FileName As String
    Dim I As Integer

    ' Check if the experience database exists, and if it doesn't, create it.
    If Not FileExists("Experience.ini") Then

        ' Cache the file name.
        FileName = App.Path & "\Experience.ini"

        ' Save the experience, for each level, into the database.
        For I = 1 To MAX_LEVEL
            Call SetStatus("Saving Experience... " & CStr(I) & "/" & CStr(MAX_LEVEL))
            Call PutVar(FileName, "EXPERIENCE", "Exp" & CStr(I), CStr(I * 1500))
        Next I
    End If
End Sub

Public Sub ClearExperience()
    Dim I As Integer
...
Public Sub LoadEmoticon()
    Dim FileName As String
    Dim I As Integer

    ' Check and make sure the emoticon database exists.
    Call CheckEmoticon

    ' Cache the file name.
    FileName = App.Path & "\Emoticons.ini"

    ' Load the emoticons into memory.
    For I = 0 To MAX_EMOTICONS
        Call SetStatus("Loading Emoticons... " & CStr(I) & "/ " & CStr(MAX_EMOTICONS))

        Emoticons(I).Pic = Val(GetVar(FileName, "EMOTICONS", "Emoticon" & CStr(I)))
        Emoticons(I).Command = GetVar(FileName, "EMOTICONS", "EmoticonC" & CStr(I))
    Next I
End Sub

Public Sub SaveEmoticon(ByVal EmoNum As Long)
    Dim FileName As String

    ' Cache the file name.
    FileName = App.Path & "\Emoticons.ini"

    ' Save the emoticon into the database.
    Call PutVar(FileName, "EMOTICONS", "Emoticon" & CStr(EmoNum), CLng(Emoticons(EmoNum).Pic))
    Call PutVar(FileName, "EMOTICONS", "EmoticonC" & CStr(EmoNum), Trim$(Emoticons(EmoNum).Command))
End Sub

Public Sub CheckEmoticon()
    Dim FileName As String
    Dim I As Integer

    ' Check if the emoticon database exists, and if it doesn't, create it.
    If Not FileExists("Emoticons.ini") Then

        ' Cache the file name.
        FileName = App.Path & "\Emoticon.ini"

        ' Save the emoticons into the database.
        For I = 0 To MAX_EMOTICONS
            Call SetStatus("Saving Emoticons... " & CStr(I) & "/" & CStr(MAX_EMOTICONS))

            Call PutVar(App.Path & "\Emoticons.ini", "EMOTICONS", "Emoticon" & CStr(I), CStr(0))
            Call PutVar(App.Path & "\Emoticons.ini", "EMOTICONS", "EmoticonC" & CStr(I), vbNullString)
        Next I
    End If
End Sub

Public Sub ClearEmoticon()
    Dim I As Integer

    ' Clear the emoticon database.
    For I = 0 To MAX_EMOTICONS
        Emoticons(I).Pic = 0
...
Sub LoadElements()
    ...
    For I = 0 To MAX_ELEMENTS
        ...
        Element(I).Strong = CLng(GetVar(FileName, "ELEMENTS", "ElementStrong" & I))
        Element(I).Weak = CLng(GetVar(FileName, "ELEMENTS", "ElementWeak" & I))
    Next I
    Exit Sub
...
Sub SaveElement(ByVal ElementNum As Long)
    ...
    Call PutVar(FileName, "ELEMENTS", "ElementStrong" & ElementNum, CLng(Element(ElementNum).Strong))
    Call PutVar(FileName, "ELEMENTS", "ElementWeak" & ElementNum, CLng(Element(ElementNum).Weak))
End Sub
...
PlayerErr:
    Call TextAdd(frmServer.txtText(7), "[DB Error] Failed to load player data for account '" & Name & "'.", True)
    Call AlertMsg(Index, "You have been disconnected from the server.")
End Sub
...
Sub AddChar(ByVal Index As Long, ByVal Name As String, ByVal Sex As Byte, ByVal ClassNum As Byte, ByVal CharNum As Long, ByVal headc As Long, ByVal bodyc As Long, ByVal logc As Long)
    ...
    If Trim$(Player(Index).Char(CharNum).Name) = vbNullString Then
        ...
        For I = 1 To MAX_INV
            If ClassData(ClassNum).ClassInv(I).ItemNum > 0 Then
                Call SetPlayerInvItemNum(Index, I, ClassData(ClassNum).ClassInv(I).ItemNum)
                Call SetPlayerInvItemValue(Index, I, ClassData(ClassNum).ClassInv(I).ItemAmt)
                Call SetPlayerInvItemDur(Index, I, Item(ClassData(ClassNum).ClassInv(I).ItemNum).Data1)
            End If
        Next I
...
Public Function FindChar(ByVal Name As String) As Boolean
    ...
    ' Cache the given character name.
    Name = Trim$(LCase$(Name))

    ' Get a free file number.
    FileID = FreeFile()

    ' Open the character list file.
    Open App.Path & "\Accounts\CharList.txt" For Input As #FileID

    ' Loop through all of the used character names.
    Do While Not EOF(FileID)
        Input #FileID, CharName

        ' Compare the character names.
        If Trim$(LCase$(CharName)) = Name Then
            FindChar = True
            Exit Do
        End If
    Loop
...
Public Sub LoadClasses()
    ...
    If MAX_CLASSES = -1 Then
        ...
        If CLASSES = 1 Then
            Call MsgBox("No class file(s) found, but classes are enabled. Building temporary class file.")
        End If
    End If
    ...
    For I = 0 To MAX_CLASSES
        ...
        On Local Error Resume Next ' used if next line tries to divide by 0
        ...
        If Not FileExists("\Classes\Class" & I & ".ini") Then
            Call PutVar(FileName, "CLASS", "Name", "Test Class")
            Call PutVar(FileName, "CLASS", "MaleSprite", CStr(0))
            Call PutVar(FileName, "CLASS", "FemaleSprite", CStr(1))
            Call PutVar(FileName, "CLASS", "Description", "This is a test class.")
            Call PutVar(FileName, "CLASS", "STR", CStr(0))
            Call PutVar(FileName, "CLASS", "DEF", CStr(0))
            Call PutVar(FileName, "CLASS", "SPEED", CStr(0))
            Call PutVar(FileName, "CLASS", "MAGI", CStr(0))
            Call PutVar(FileName, "CLASS", "MAP", CStr(1))
            Call PutVar(FileName, "CLASS", "X", CStr(0))
            Call PutVar(FileName, "CLASS", "Y", CStr(0))
            Call PutVar(FileName, "CLASS", "Locked", CStr(0))
            
            For cItem = 1 To MAX_INV
                Call PutVar(FileName, "CLASS_DEFITEMS", "ItemNum_" & CStr(cItem), CStr(0))
                Call PutVar(FileName, "CLASS_DEFITEMS", "ItemAmt_" & CStr(cItem), CStr(0))
            Next cItem
        End If
        ...
        ClassData(I).Desc = GetVar(FileName, "CLASS", "Description")
        ...
        ClassData(I).X = CLng(GetVar(FileName, "CLASS", "X"))
        ClassData(I).Y = CLng(GetVar(FileName, "CLASS", "Y"))
        ClassData(I).Locked = CLng(GetVar(FileName, "CLASS", "Locked"))

        For cItem = 1 To MAX_INV
            ClassData(I).ClassInv(cItem).ItemNum = CLng(GetVar(FileName, "CLASS_DEFITEMS", "ItemNum_" & CStr(cItem)))
            ClassData(I).ClassInv(cItem).ItemAmt = CLng(GetVar(FileName, "CLASS_DEFITEMS", "ItemAmt_" & CStr(cItem)))
        Next cItem
    Next I
    Exit Sub

ClassErr:
    Call MsgBox("Failed to load the file: 'Classes\Class" & I & ".ini'. Check to make sure all variables exist.")
    Call DestroyServer
    End
End Sub
...
Sub LoadGuilds()
    ...
    For I = 1 To MAX_GUILDS
        Temp = I / MAX_GUILDS * 100
        Call SetStatus("Loading Guilds... " & Temp & "%")

        Guild(I).Name = Trim$(ReadINI(Val(I), "Name", App.Path & "\Guilds.ini", vbNullString))
        Guild(I).Founder = Trim$(ReadINI(Val(I), "Name", App.Path & "\Guilds.ini", vbNullString))
    Next I
End Sub
...
Public Sub AddLog(ByVal LogMessage As String, ByVal FileName As String)
...
Sub SaveArrow(ByVal ArrowNum As Long)
    ...
    Call PutVar(FileName, "Arrow" & ArrowNum, "ArrowPic", CLng(Arrows(ArrowNum).Pic))
    Call PutVar(FileName, "Arrow" & ArrowNum, "ArrowRange", CLng(Arrows(ArrowNum).Range))
    Call PutVar(FileName, "Arrow" & ArrowNum, "ArrowAmount", CLng(Arrows(ArrowNum).Amount))
End Sub
