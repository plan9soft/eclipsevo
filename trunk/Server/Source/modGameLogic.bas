...
Function FindOpenMapItemSlot(ByVal MapNum As Long) As Long
    ...
    ' TODO: Instead of ignoring the item created by a player or NPC, re-use the
    '       map array and keep track of the last item removed by the server. [Mellowz]
...
Public Sub SpawnItem(ByVal ItemNum As Long, ByVal ItemVal As Long, ByVal MapNum As Long, ByVal X As Long, ByVal Y As Long)
    ...
    ' Check if all map item slots are full.
...
Public Sub SpawnItemSlot(ByVal MapItemSlot As Long, ByVal ItemNum As Long, ByVal ItemVal As Long, ByVal ItemDur As Long, ByVal MapNum As Long, ByVal X As Long, ByVal Y As Long)
    ...
    ' Check for subscript out of range.
    If MapItemSlot < 1 Or MapItemSlot > MAX_MAP_ITEMS Then Exit Sub

    ' Create the item server-side.
    MapItem(MapNum, MapItemSlot).num = ItemNum
    MapItem(MapNum, MapItemSlot).Value = ItemVal

    If (Item(ItemNum).Type >= ITEM_TYPE_WEAPON) And (Item(ItemNum).Type <= ITEM_TYPE_NECKLACE) Then
        MapItem(MapNum, MapItemSlot).Dur = ItemDur
    Else
        MapItem(MapNum, MapItemSlot).Dur = 0
    End If

    MapItem(MapNum, MapItemSlot).X = X
    MapItem(MapNum, MapItemSlot).Y = Y

    ' Create the item client-side.
    Call SendDataToMap(MapNum, POut.SpawnItem & SEP_CHAR & MapItemSlot & SEP_CHAR & ItemNum & SEP_CHAR & ItemVal & SEP_CHAR & MapItem(MapNum, MapItemSlot).Dur & SEP_CHAR & X & SEP_CHAR & Y & END_CHAR)
End Sub
...
Sub PlayerMapDropItem(ByVal Index As Long, ByVal InvNum As Long, ByVal Amount As Long)
    ...
    If (GetPlayerInvItemNum(Index, InvNum) > 0) Then
        If (GetPlayerInvItemNum(Index, InvNum) <= MAX_ITEMS) Then
            ...
            Else
                Call PlayerMsg(Index, "There are too many items already on the ground.", BRIGHTRED)
            End If
        End If
    ...
    If NPCnum > 0 Then
        If GameTime = TIME_NIGHT Then
            If NPC(NPCnum).SpawnTime = 1 Then
                ...
                Call SendDataToMap(MapNum, POut.NPCDead & SEP_CHAR & MapNpcNum & END_CHAR)
                Exit Sub
            End If
        Else
        ...
        If Spawned Then
            packet = POut.SpawnNPC & SEP_CHAR & MapNpcNum & SEP_CHAR & MapNPC(MapNum, MapNpcNum).num & SEP_CHAR & MapNPC(MapNum, MapNpcNum).X & SEP_CHAR & MapNPC(MapNum, MapNpcNum).Y & SEP_CHAR & MapNPC(MapNum, MapNpcNum).Dir & SEP_CHAR & NPC(MapNPC(MapNum, MapNpcNum).num).Big & END_CHAR
            Call SendDataToMap(MapNum, packet)
        End If
    End If

    ' Enable this to display HP when monsters spawn.
    ' Call SendDataToMap(MapNum, POut.NPCHP & SEP_CHAR & MapNpcNum & SEP_CHAR & MapNPC(MapNum, MapNpcNum).HP & SEP_CHAR & GetNpcMaxHP(MapNPC(MapNum, MapNpcNum).num) & END_CHAR)
End Sub
...
Function CanAttackPlayer(ByVal Attacker As Long, ByVal Victim As Long) As Boolean
    ...
    If (GetPlayerMap(Attacker) = GetPlayerMap(Victim)) And (GetTickCount > Player(Attacker).AttackTimer + AttackSpeed) Then
        PX = GetPlayerX(Attacker)
        PY = GetPlayerY(Attacker)
        ...
        If (GetPlayerY(Victim) = PY) And (GetPlayerX(Victim) = PX) Then
...
Function CanAttackNpc(ByVal Attacker As Long, ByVal MapNpcNum As Long) As Boolean
    ...
    If GetTickCount > Player(Attacker).AttackTimer + AttackSpeed Then
        ...
        If (MapNPC(MapNum, MapNpcNum).Y = PY) And (MapNPC(MapNum, MapNpcNum).X = PX) Then
...
Sub AttackPlayer(ByVal Attacker As Long, ByVal Victim As Long, ByVal Damage As Long)
    ...
    Call SendDataToMapBut(Attacker, GetPlayerMap(Attacker), POut.Attack & SEP_CHAR & Attacker & END_CHAR)
        ...
        If Damage >= GetPlayerHP(Victim) Then
            ...
            Else
                ...
                Else
                    ...
                    Else
                        ClassID = GetPlayerClass(Victim)
        
                        Call PlayerWarp(Victim, GetClassSpawnMap(ClassID), GetClassSpawnX(ClassID), GetClassSpawnY(ClassID))
                    End If
                End If
            End If
    ...
    Call SendDataToMap(GetPlayerMap(Victim), POut.Sound & SEP_CHAR & "pain" & SEP_CHAR & Player(Victim).Char(Player(Victim).CharNum).Sex & END_CHAR)
End Sub
...
Sub NpcAttackPlayer(ByVal MapNpcNum As Long, ByVal Victim As Long, ByVal Damage As Long)
    ...
    ' Send this packet so they can see the person attacking
    Call SendDataToMap(GetPlayerMap(Victim), POut.NPCAttack & SEP_CHAR & MapNpcNum & END_CHAR)
    ...
    Call SendDataTo(Victim, POut.DrawNPCDamage & SEP_CHAR & Damage & END_CHAR)
    Call SendDataToMap(GetPlayerMap(Victim), POut.Sound & SEP_CHAR & "pain" & SEP_CHAR & Player(Victim).Char(Player(Victim).CharNum).Sex & END_CHAR)
End Sub
...
Sub AttackNpc(ByVal Attacker As Long, ByVal MapNpcNum As Long, ByVal Damage As Long)
    Dim Name As String
    Dim Exp As Long
    Dim n As Long, I As Long, Count As Integer, X As Long
    ...
    If Damage >= MapNPC(MapNum, MapNpcNum).HP Then
        ...
        ' Calculate exp to give attacker
        If Add > 0 Then
            Exp = CLng(NPC(NPCnum).Exp + (NPC(NPCnum).Exp * CLng(Add)))
        Else
            Exp = CLng(NPC(NPCnum).Exp)
        End If
        ...
        If Not Player(Attacker).InParty Then
            ...
            End If
        Else
            If Party(Player(Attacker).PartyIndex).EXPMethod = 0 Then ' Don't share.
                If GetPlayerLevel(Attacker) = MAX_LEVEL Then
                    Call SetPlayerExp(Attacker, Experience(MAX_LEVEL))
                    Call BattleMsg(Attacker, "You can't gain anymore experience!", BRIGHTBLUE, 0)
                Else
                ...
                End If
            Else
                Count = Party_GetPartyCount(Attacker)
                ...
                For X = 1 To 8
                    If Party(Player(Attacker).PartyIndex).Members(X) > 0 Then
                        If GetPlayerLevel(Party(Player(Attacker).PartyIndex).Members(X)) = MAX_LEVEL Then
                            Call SetPlayerExp(Party(Player(Attacker).PartyIndex).Members(X), Experience(MAX_LEVEL))
                            Call BattleMsg(Party(Player(Attacker).PartyIndex).Members(X), "You can't gain anymore experience!", BRIGHTBLUE, 0)
                        Else
                            Call SetPlayerExp(Party(Player(Attacker).PartyIndex).Members(X), GetPlayerExp(Party(Player(Attacker).PartyIndex).Members(X)) + Exp)
                            Call BattleMsg(Party(Player(Attacker).PartyIndex).Members(X), "You gained " & Exp & " party experience.", BRIGHTBLUE, 0)
                        End If
        ...
        If Player(Attacker).InParty Then
            For X = 1 To MAX_PARTY_MEMBERS
                Call CheckPlayerLevelUp(Party(Player(Attacker).PartyIndex).Members(X))
            Next X
        End If
...
Sub PlayerMove(ByVal Index As Long, ByVal Dir As Long, ByVal Movement As Long)
    ...
    If Y < 0 Then
        If Map(GetPlayerMap(Index)).Up > 0 Then
            Call PlayerWarp(Index, Map(GetPlayerMap(Index)).Up, X, MAX_MAPY)
            Moved = YES
        End If
    End If
    ...
    If X < 0 Then
        If Map(GetPlayerMap(Index)).Left > 0 Then
            Call PlayerWarp(Index, Map(GetPlayerMap(Index)).Left, MAX_MAPX, Y)
            Moved = YES
        End If
    End If
    ...
    If Not Moved = YES Then
        ...
        If Map(GetPlayerMap(Index)).Tile(X, Y).Type <> TILE_TYPE_BLOCKED Then
            ...
            If (Map(GetPlayerMap(Index)).Tile(X, Y).Type <> TILE_TYPE_KEY Or Map(GetPlayerMap(Index)).Tile(X, Y).Type <> TILE_TYPE_DOOR) Or ((Map(GetPlayerMap(Index)).Tile(X, Y).Type = TILE_TYPE_DOOR Or Map(GetPlayerMap(Index)).Tile(X, Y).Type = TILE_TYPE_KEY) And TempTile(GetPlayerMap(Index)).DoorOpen(X, Y) = YES) Then
                ...
                packet = POut.PlayerMove & SEP_CHAR & Index & SEP_CHAR & GetPlayerX(Index) & SEP_CHAR & GetPlayerY(Index) & SEP_CHAR & GetPlayerDir(Index) & SEP_CHAR & Movement & END_CHAR
    ...
    If GetPlayerX(Index) + 1 <= MAX_MAPX Then
        If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) + 1, GetPlayerY(Index)).Type = TILE_TYPE_DOOR Then
            ...
            If TempTile(GetPlayerMap(Index)).DoorOpen(X, Y) = NO Then
                ...
                Call SendDataToMap(GetPlayerMap(Index), POut.Sound & SEP_CHAR & "door" & END_CHAR)
            End If
        End If
    End If
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Type = TILE_TYPE_SHOP Then
        If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Data1 > 0 Then
            If Shop(Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Data1).ShopItem(1).ItemNum > 0 Then
                Call SendTrade(Index, Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Data1)
            Else
                Call PlayerMsg(Index, "This shop has no items.", BRIGHTRED)
            End If
        Else
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Type = TILE_TYPE_SPRITE_CHANGE Then
        ...
        Else
            If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Data2 = 0 Then
                Call SendDataTo(Index, POut.SpriteChange & SEP_CHAR & 0 & END_CHAR)
            Else
                ...
                End If
                Call SendDataTo(Index, POut.SpriteChange & SEP_CHAR & 1 & END_CHAR)
            End If
        End If
    End If
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Type = TILE_TYPE_NOTICE Then
        ...
        If Not Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).String3 = vbNullString Or Not Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).String3 = vbNullString Then
            Call SendDataToMap(GetPlayerMap(Index), POut.Sound & SEP_CHAR & "soundattribute" & SEP_CHAR & Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).String3 & END_CHAR)
        End If
    End If
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Type = TILE_TYPE_SOUND Then
        Call SendDataToMap(GetPlayerMap(Index), POut.Sound & SEP_CHAR & "soundattribute" & SEP_CHAR & Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).String1 & END_CHAR)
    End If
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).Type = TILE_TYPE_BANK Then
        Call SendDataTo(Index, POut.OpenBank & END_CHAR)
    End If
...
Function CanNpcMove(ByVal MapNum As Long, ByVal MapNpcNum As Long, ByVal Dir) As Boolean
    ...
    If Y < 0 Or Y > MAX_MAPY Or X < 0 Or X > MAX_MAPX Then
        CanNpcMove = False
        Exit Function
    End If
    ...
    ' Check to make sure that the tile is walkable.
    If TileType = TILE_TYPE_BLOCKED Or TileType = TILE_TYPE_NPCAVOID Or TileType = TILE_TYPE_GUILDBLOCK _
    Or TileType = TILE_TYPE_SIGN Or TileType = TILE_TYPE_CBLOCK Or TileType = TILE_TYPE_ROOFBLOCK Or TileType = TILE_TYPE_ONCLICK Or _
    ((TileType = TILE_TYPE_KEY Or TileType = TILE_TYPE_DOOR) And TempTile(MapNum).DoorOpen(X, Y) = NO) Then
        CanNpcMove = False
        Exit Function
    End If
...
Public Sub NpcMove(ByVal MapNum As Long, ByVal MapNpcNum As Long, ByVal Dir As Long, ByVal Movement As Long)
    ...
    Call SendDataToMap(MapNum, POut.NpcMove & SEP_CHAR & MapNpcNum & SEP_CHAR & MapNPC(MapNum, MapNpcNum).X & SEP_CHAR & MapNPC(MapNum, MapNpcNum).Y & SEP_CHAR & MapNPC(MapNum, MapNpcNum).Dir & SEP_CHAR & Movement & END_CHAR)
End Sub
...
Public Sub NpcDir(ByVal MapNum As Long, ByVal MapNpcNum As Long, ByVal Dir As Long)
    ...
    Call SendDataToMap(MapNum, POut.NPCDirection & SEP_CHAR & MapNpcNum & SEP_CHAR & Dir & END_CHAR)
End Sub
...
Public Sub JoinGame(ByVal Index As Long)
    ...
    Call SendDataTo(Index, POut.LoginOK & SEP_CHAR & Index & END_CHAR)
    ...
    Else
        ...
        If FileExists("MOTD.ini") Then
            MOTD = Trim$(GetVar(App.Path & "\MOTD.ini", "MOTD", "Msg"))
            If LenB(MOTD) <> 0 Then
                Call PlayerMsg(Index, "MOTD: " & MOTD, 11)
            End If
        Else
            Call PlayerMsg(Index, "MOTD: File not found.", 11)
        End If
    ...
    Call SendDataTo(Index, POut.InGame & END_CHAR)
...
Public Sub LeftGame(ByVal Index As Long)
    ...
    If Player(Index).InGame Then
        ...
        For X = 1 To MAX_MAP_NPCS
            If MapNPC(GetPlayerMap(Index), X).Target = Index Then
                MapNPC(GetPlayerMap(Index), X).Target = 0
            End If
        Next X
        ...
        ' Check if the player is in a party and remove them.
        If Player(Index).InParty Then Call Packet_PartyLeave(Index)

        ' Check if the player is in a conversation and close it.
        If Player(Index).InChat = 1 Then Call Packet_QuitChat(Index)
...
Sub CheckPlayerLevelUp(ByVal Index As Long)
    ...
    If GetPlayerExp(Index) >= GetPlayerNextLevel(Index) Then
        If GetPlayerLevel(Index) < MAX_LEVEL Then
            ...
            End If
            Call SendDataToMap(GetPlayerMap(Index), POut.LevelUp & SEP_CHAR & Index & END_CHAR)
...
Function CanAttackPlayerWithArrow(ByVal Attacker As Long, ByVal Victim As Long) As Boolean
    ...
    If Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NONE Or Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NO_PENALTY Or GetPlayerPK(Victim) = YES Then
        ' Make sure they are high enough level
        If GetPlayerLevel(Attacker) < PKMINLVL Then
            Call PlayerMsg(Attacker, "You are below level " & PKMINLVL & ". You cannot attack other players yet.", BRIGHTRED)
        Else
            If GetPlayerLevel(Victim) < PKMINLVL Then
                Call PlayerMsg(Attacker, GetPlayerName(Victim) & " is below level " & PKMINLVL & ". He cannot be attacked.", BRIGHTRED)
            Else
                If Trim$(GetPlayerGuild(Attacker)) <> vbNullString And GetPlayerGuild(Victim) <> vbNullString Then
                    If Trim$(GetPlayerGuild(Attacker)) <> Trim$(GetPlayerGuild(Victim)) Then
                        CanAttackPlayerWithArrow = True
                    Else
                        Call PlayerMsg(Attacker, "You cannot attack guild members.", BRIGHTRED)
                    End If
                Else
                    CanAttackPlayerWithArrow = True
                End If
...
Sub SendIndexWornEquipmentto(ByVal Index As Long, ByVal From As Long)
    ...
    Call SendDataTo(Index, POut.ItemWorn & SEP_CHAR & From & SEP_CHAR & Armor & SEP_CHAR & Weapon & SEP_CHAR & Helmet & SEP_CHAR & Shield & SEP_CHAR & Legs & SEP_CHAR & Ring & SEP_CHAR & Necklace & END_CHAR)
End Sub
    ...
    For I = 1 To MAX_PLAYERS
        If IsPlaying(I) = True Then
            If GetPlayerMap(I) = GetPlayerMap(Index) Then
                ...
                packet = POut.ItemWorn & SEP_CHAR & I & SEP_CHAR & Armor & SEP_CHAR & Weapon & SEP_CHAR & Helmet & SEP_CHAR & Shield & SEP_CHAR & Legs & SEP_CHAR & Ring & SEP_CHAR & Necklace & END_CHAR
...
Public Sub TimerCreate(ByVal Name As String, ByVal Interval As Long)
    On Error Resume Next

    Dim TimerDesc As clsCTimers

    ' Create a new instance.
    Set TimerDesc = New clsCTimers

    ' Create the description.
    TimerDesc.Name = Name
    TimerDesc.Interval = Interval
    TimerDesc.tmrWait = GetTickCount + Interval

    ' Create the timer.
    Call CTimers.Add(TimerDesc, Name)
End Sub

Public Sub TimerRemove(ByVal Name As String)
    On Error Resume Next
...
Public Function TimerGetTime(ByVal Name As String) As Long
    On Error Resume Next

    ' Get the remaining time on the timer.
    TimerGetTime = CTimers.Item(Name).tmrWait - GetTickCount
End Function
...
Sub ScriptSetTile(ByVal mapper As Long, ByVal X As Long, ByVal Y As Long, ByVal setx As Long, ByVal sety As Long, ByVal tileset As Long, ByVal layer As Long)
    Dim packet As String
    packet = POut.TileCheck & SEP_CHAR & mapper & SEP_CHAR & X & SEP_CHAR & Y & SEP_CHAR & layer & SEP_CHAR
...
Sub ScriptSetAttribute(ByVal mapper As Long, ByVal X As Long, ByVal Y As Long, ByVal Attrib As Long, ByVal Data1 As Long, ByVal Data2 As Long, ByVal Data3 As Long, ByVal String1 As String, ByVal String2 As String, ByVal String3 As String)
    ...
    packet = POut.TileCheckAttribute & SEP_CHAR & mapper & SEP_CHAR & X & SEP_CHAR & Y & SEP_CHAR
    With Map(mapper).Tile(X, Y)
...
' Skill System - Returns an answer based on the string supplied.
' Note: This function was created so scripting can be disabled.
Public Function Evaluate(ByVal Equation As String) As Long
    Dim ScriptObject As ScriptControl

    On Error Resume Next

    ' Create the script object.
    Set ScriptObject = New ScriptControl

    ' Set the language to VBScript.
    ScriptObject.Language = "VBScript"

    ' Wow! That's a big number!
    Evaluate = CLng(ScriptObject.Eval(Equation))
End Function

' Checks if the guild name already exists.
Public Function GuildExists(ByVal GuildName As String) As Boolean
    Dim I As Long

    For I = 1 To MAX_GUILDS
        If Guild(I).Name = GuildName Then
            GuildExists = True
        End If
    Next I
End Function

Public Function Party_FindOpenPartySlot() As Long
    Dim I As Long

    For I = 1 To UBound(Party)
        If Not Party(I).InUse Then
            Party_FindOpenPartySlot = I
            Exit Function
        End If
    Next I
End Function

Public Function Party_FindOpenPlayerSlot(ByVal PartyID As Long) As Long
    Dim I As Long

    For I = 1 To 8
        If Party(PartyID).Members(I) = 0 Then
            Party_FindOpenPlayerSlot = I
            Exit Function
        End If
    Next I
End Function

Public Function Party_GetPlayerSlot(ByVal Index As Long)
    Dim I As Long

    For I = 1 To 8
        If Party(Player(Index).PartyIndex).Members(I) = Index Then
            Party_GetPlayerSlot = I
            Exit Function
        End If
    Next I
End Function

Public Function Party_GetPartyCount(ByVal Index As Long)
    Dim Count As Long
    Dim I As Long

    For I = 1 To 8
        If Not Party(Player(Index).PartyIndex).Members(I) = 0 Then
            Count = Count + 1
        End If
    Next I

    Party_GetPartyCount = Count
End Function

Public Sub PartyMsg(ByVal PartyID As Long, ByVal Message As String)
    Dim I As Long

    For I = 1 To 8
        If Not Party(PartyID).Members(I) = 0 Then
            Call PlayerMsg(Party(PartyID).Members(I), Message, GREEN)
        End If
    Next I
End Sub
