...
Begin VB.Form frmMirage 
   ...
   Begin VB.PictureBox picOptions 
      ...
      Visible         =   0   'False
      Width           =   2655
      Begin VB.CheckBox chkNightEffect 
         Appearance      =   0  'Flat
         BackColor       =   &H80000005&
         Caption         =   "Night Effect"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   6.75
            Charset         =   0
         ...
         Width           =   2325
      End
      Begin VB.CheckBox chkSwearFilter 
         Appearance      =   0  'Flat
         BackColor       =   &H80000005&
...
Private Sub chkNightEffect_Click()
    Call WriteINI("CONFIG", "NightEffect", chkNightEffect.Value, App.Path & "\Config.ini")
End Sub

Private Sub chkSwearFilter_Click()
    Call WriteINI("CONFIG", "SwearFilter", chkSwearFilter.Value, App.Path & "\Config.ini")

    ' Check if it's enabled.
    If chkSwearFilter.Value = vbChecked Then
...
Private Sub Form_Load()
    Call GUI_PictureLoad(frmMirage, "GUI\800x600")
End Sub
...
Private Sub lblOptions_Click()
    ...
    chkSwearFilter.Value = Trim$(ReadINI("CONFIG", "SwearFilter", App.Path & "\Config.ini"))
...
Private Sub picInv_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, y As Single)
    ...
    SelectedItem.Top = picInv(Inventory).Top - 1
    SelectedItem.Left = picInv(Inventory).Left - 1
    ...
    Else
        ...
        If frmNewShop.SellItems And frmNewShop.Visible = True Then
            If Item(GetPlayerInvItemNum(MyIndex, Inventory)).Stackable = YES Then
                AMT = Val(InputBox("How many would you like to sell?", "Sell Items"))
                If AMT > 0 Then
                    ' Sell the items
...
Private Sub picInv_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, y As Single)
    ...
    If Player(MyIndex).Inv(d).num > 0 Then
        If Item(GetPlayerInvItemNum(MyIndex, d)).Type = ITEM_TYPE_CURRENCY Or Item(GetPlayerInvItemNum(MyIndex, d)).Stackable = 1 Then
            If Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).desc) = vbNullString Then
                itmDesc.Top = 224
                itmDesc.Height = 17
            Else
                itmDesc.Top = 8
        ...
        End If
        If Item(GetPlayerInvItemNum(MyIndex, d)).Type = ITEM_TYPE_CURRENCY Or Item(GetPlayerInvItemNum(MyIndex, d)).Stackable = 1 Then
            descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (" & GetPlayerInvItemValue(MyIndex, d) & ")"
        Else
            If GetPlayerWeaponSlot(MyIndex) = d Then
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (Worn)"
            ElseIf GetPlayerArmorSlot(MyIndex) = d Then
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (Worn)"
            ElseIf GetPlayerHelmetSlot(MyIndex) = d Then
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (Worn)"
            ElseIf GetPlayerShieldSlot(MyIndex) = d Then
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (Worn)"
            ElseIf GetPlayerLegsSlot(MyIndex) = d Then
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (Worn)"
            ElseIf GetPlayerRingSlot(MyIndex) = d Then
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (Worn)"
            ElseIf GetPlayerNecklaceSlot(MyIndex) = d Then
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name) & " (Worn)"
            Else
                descName.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).Name)
            End If
        End If

        descStr.Caption = Item(GetPlayerInvItemNum(MyIndex, d)).StrReq & " " & STAT1
        descDef.Caption = Item(GetPlayerInvItemNum(MyIndex, d)).DefReq & " " & STAT2
        descSpeed.Caption = Item(GetPlayerInvItemNum(MyIndex, d)).SpeedReq & " " & STAT3
        descMagic.Caption = Item(GetPlayerInvItemNum(MyIndex, d)).MagicReq & " " & STAT4
        descHpMp.Caption = "HP: " & Item(GetPlayerInvItemNum(MyIndex, d)).AddHP & " MP: " & Item(GetPlayerInvItemNum(MyIndex, d)).AddMP & " SP: " & Item(GetPlayerInvItemNum(MyIndex, d)).AddSP
        descSD.Caption = STAT1 & ": " & Item(GetPlayerInvItemNum(MyIndex, d)).AddSTR & " " & STAT2 & ": " & Item(GetPlayerInvItemNum(MyIndex, d)).AddDEF
        descMS.Caption = STAT3 & ": " & Item(GetPlayerInvItemNum(MyIndex, d)).AddMAGI & " " & STAT4 & ": " & Item(GetPlayerInvItemNum(MyIndex, d)).AddSpeed
        desc.Caption = Trim$(Item(GetPlayerInvItemNum(MyIndex, d)).desc)
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
        ...
        If SpellMemorized > 0 Then
            If GetTickCount > Player(MyIndex).AttackTimer + 1000 Then
                If Player(MyIndex).Moving = 0 Then
                    Call SendData(POut.SpellCast & SEP_CHAR & SpellMemorized & END_CHAR)
    ...
    If KeyCode = vbKeyHome Then
        If Player(MyIndex).Moving = NO Then
            If GetPlayerDir(MyIndex) = DIR_UP Then Call SetPlayerDir(MyIndex, DIR_RIGHT)
            If GetPlayerDir(MyIndex) = DIR_RIGHT Then Call SetPlayerDir(MyIndex, DIR_DOWN)
            If GetPlayerDir(MyIndex) = DIR_DOWN Then Call SetPlayerDir(MyIndex, DIR_LEFT)
            If GetPlayerDir(MyIndex) = DIR_LEFT Then Call SetPlayerDir(MyIndex, DIR_UP)
...
Private Sub picScreen_MouseMove(Button As Integer, Shift As Integer, X As Single, y As Single)
    ...
    If itmDesc.Visible Then itmDesc.Visible = False

    ' Check if we're in the map editor.
    If InEditor Then
        ...
        ' Update the map editor coordinates.
...
Private Sub cmdAccess_Click()
    If IsNumeric(txtAccess.Text) Then
        Call SendChangeGuildAccess(txtName.Text, CLng(txtAccess.Text))
    End If
End Sub
